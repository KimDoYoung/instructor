    ...
    #generateCalendarHTML(year, month) {
        ...
        for (let ymd = startDay; ymd <= endDay; ymd = this.#adjustDate(ymd, 1)) {
            ...
            if (new Date(year, month - 1, day).toDateString() === new Date().toDateString()) {
                dayContent = `<div class="today-highlight">${day}</div>`; // 오늘 날짜를 뱃지로 표현
            }else {
                if(['01','07','15','21'].includes(lunarYmds[ymd].slice(6))){
                    dayContent = `${day}<span class="ml-2 small fst-italic text-secondary">(${Number(lunarYmds[ymd].slice(4,6))}.${Number(lunarYmds[ymd].slice(6))})</span>`; // 오늘 날짜를 뱃지로 표현
                }else{
                    dayContent = `${day}`; 
                }
            }
            ...
            if (isHoliday) {
                ...
                dayContent += ` <div class="badge bg-danger" style="width: 100%;">${this.holidays[ymd]}</div>`;
            }            
